...
For most real-time data needs, we recommend using our [WebSocket Market Data API](../websocket-api-reference.md) instead of polling the REST API. For order entry, the [WebSocket Order Entry API](../ws-exec-api-reference.md) lets you place and cancel orders over a persistent connection with id-correlated responses.
